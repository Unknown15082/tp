...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_PRODUCTS).isPresent()) {
            products = ParserUtil.parseProducts(argMultimap.getValue(PREFIX_PRODUCTS).get());
        }
        ...
        Deadline deadline = Deadline.empty();
        ...
        if (argMultimap.getValue(PREFIX_CONTACT).isPresent()) {
            contact = ParserUtil.parseContact(argMultimap.getValue(PREFIX_CONTACT).get());
        }

        Person person = new Person(name, products, location, deadline, contact);
